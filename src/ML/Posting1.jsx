...
class Posting1 extends Component{
    render(){
      return(
        ...
        위 그림은 정규화하기 전과 정규화를 한 후의 사진이다. 이렇게 눈으로도 보기 쉽고 실제 kNN을 사용할 때, 이런 정규화를 통해 더 높은 정확도를 가져올 수 있다.<br/><br/>
        이제 실제 실습을 통해서 kNN을 사용해보자.
        ...
        아래는 k를 1~99까지 변경해가면서 kNN모델을 생성하고, 그 모델에 학습을 시켜 train과 test set에 대한 정확도를 score이라는 pd객체에 저장을 한다.<br/><br/>
        <img src={Img7}className="code"/><br/><br/>
        이렇게 나온 k에 따른 정확도는 아래와 같은 그래프를 보이는데, 파란색이 train 빨간색이 test에 대한 결과이다.<br/><br/>
        <img src={Img8}className="code"/><br/><br/>
        파란색과 빨간색 모두 처음에는 높게 나오다가 나중에 k가 커질수록 낮게 나오는 데, 이것은 생각해보면 당연한 결과이다.<br/><br/>
        파란색의 경우는 맨처음 1일때 100%의 정확도를 보여주는데, 이건 학습된 데이터를 그대로 사용했기 때문에 당연한 결과이고 갈수록 퍼센트가 낮아지는 것을 볼 수 있다.<br/><br/>
        사실 파란색의 정확도는 거의 중요하지 않다고 봐도 무방하고 test인 빨간색이 핵심이다. 그 이유는 파란색은 이전의 알고 있는 데이터이기 때문에 이미 알고 있는 데이터를 예측하는 것은 의미가 없고, 새로 들어온다고 가정하는 빨간색 test데이터가 더 중요하기 때문이다.<br/><br/>
        빨간색의 경우 처음에는 1일때 좀 낮다가 9~12사이 쯤에 높은 정확도를 볼 수 있는데, 어느정도 높은 정확도를 갖기 위해서는 주변 이웃을 많이 검사해야하기 때문에 어느정도 높은 k값을 가져야하는 것으로 판단이 되고, 통계학적으로 11정도가 확률적으로 가장 좋은 k라고는 한다.<br/><br/>
        이상 포스팅 끝<br/><br/>
        </h4>
        </div>
      );
